...
( echo ">> fwaccel stats output below" && fwaccel stats ) | tee -a /var/log/$HOSTNAME/info.txt
...
cp $CPDIR/log/cpd.elg* /var/log/$HOSTNAME/
...
tar -cvzf "$(uname -n)-`date +"%m-%d-%Y"`.tar.gz" /var/log/$HOSTNAME/

echo "DONE! The info collected will be under /var/log/ with the name $HOSTNAME and todays date in a tar.gz"
